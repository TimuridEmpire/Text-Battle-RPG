...
public class Battle {
	...
	public static Monster createRandomMonster(Player player, String mode) 
			throws NoSuchMethodException, SecurityException, InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException 
	{
		String[] monsterTypes = {"Zombie","Slime","Witch","Giant","Troll"};
		Random randomChoice = new Random();
		String packageName = "game.textbasedRPG.";
		String randomMonster = packageName;
		 if (mode.equals("witch")) { //fun and chaotic mode
				randomMonster = packageName+"Witch";
		} else { //normal mode
			randomMonster = packageName + monsterTypes[randomChoice.nextInt(monsterTypes.length)]; //gets a random monster type from the list of monster types
		}
			try {
	        ...
	        if (player.getLevel() > 1) {
	        	monsterHP = (int) (player.getMinDmg()*(player.getLevel()*0.75)*randomFactor);
	        }
		...
		return new Monster("Generic Monster");
	...
	public static boolean validityCheck(String input) {
		try {
			...
			return (intInput >= 0); // && intInput <= 5 (for if using array of length 5)
		} catch (Exception e) {
